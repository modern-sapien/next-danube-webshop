...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (isLogin) {
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_ENVIRONMENT_URL}api/v1/auth/login`, {
          method: "POST",
          headers: {
      ...
      try {
        const response = await fetch(`${process.env.NEXT_PUBLIC_ENVIRONMENT_URL}api/v1/auth/register`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
